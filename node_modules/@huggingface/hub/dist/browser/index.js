...
var XetBlob = (_class2 = class extends Blob {
  ...
  #loadReconstructionInfo() {
    ...
    this.#reconstructionInfoPromise = (async () => {
      ...
      const resp = await this.fetch(_nullishCoalesce(this.reconstructionUrl, () => ( `${connParams.casUrl}/v1/reconstruction/${this.hash}`)), {
        headers: {
            ...
            while (totalBytesRead < maxBytes && result.value.byteLength) {
              ...
              if (shouldYield) {
                ...
                if (uncompressed.byteLength) {
                  ...
                  _optionalChain([listener, 'optionalCall', _68 => _68({ event: "progress", progress: { read: totalBytesRead, total: maxBytes } })]);
                }
              }
              chunkIndex++;
...
async function* createXorbs(fileSources, params) {
  const chunkModule = await Promise.resolve().then(() => _interopRequireWildcard(require("./chunker_wasm-HE56V7BK.js")));
  let xorbId = 0;
...
function backtrackDedup(xorb, computeHmac, shardData, chunkCache, chunkMetadata, dedupedBytes) {
  ...
  for (let chunkToRecheckIndex = _nullishCoalesce(_optionalChain([xorb, 'access', _69 => _69.immutableData, 'optionalAccess', _70 => _70.chunkIndex]), () => ( 0)); chunkToRecheckIndex < xorb.chunks.length; chunkToRecheckIndex++) {
  ...
  try {
    const allOperations = (await Promise.all(
      params.operations.map(async (operation) => {
        ...
        _optionalChain([abortSignal, 'optionalAccess', _71 => _71.throwIfAborted, 'call', _72 => _72()]);
    ...
    const gitAttributes = _optionalChain([allOperations, 'access', _73 => _73.filter, 'call', _74 => _74(isFileOperation), 'access', _75 => _75.find, 'call', _76 => _76((op) => op.path === ".gitattributes"), 'optionalAccess', _77 => _77.content]);
    for (const operations of chunk(allOperations.filter(isFileOperation), 100)) {
      ...
      _optionalChain([abortSignal, 'optionalAccess', _78 => _78.throwIfAborted, 'call', _79 => _79()]);
    ...
    for (const operations of chunk(
      ...
      _optionalChain([abortSignal, 'optionalAccess', _80 => _80.throwIfAborted, 'call', _81 => _81()]);
      const payload = {
        operation: "upload",
      ...
      if (params.useXet) {
        for (const obj of json.objects) {
          ...
          if (!_optionalChain([obj, 'access', _82 => _82.actions, 'optionalAccess', _83 => _83.upload])) {
            yield {
              event: "fileProgress",
              path: op.path,
              progress: 1,
        ...
        for await (const event of uploadShards(
          async function* () {
            for (const obj of json.objects) {
              const op = shaToOperation.get(obj.oid);
              if (!op || !_optionalChain([obj, 'access', _84 => _84.actions, 'optionalAccess', _85 => _85.upload])) {
                continue;
              }
          ...
          {
            customFetch: _nullishCoalesce(params.fetch, () => ( fetch)),
            accessToken,
        ...
        yield* eventToGenerator((yieldCallback, returnCallback, rejectCallback) => {
          return promisesQueueStreaming(
            json.objects.map((obj) => async () => {
              ...
              if (_optionalChain([header, 'optionalAccess', _92 => _92.chunk_size])) {
                ...
                await promisesQueueStreaming(
                  parts.map((part) => async () => {
                    _optionalChain([abortSignal, 'optionalAccess', _93 => _93.throwIfAborted, 'call', _94 => _94()]);
                ...
                _optionalChain([abortSignal, 'optionalAccess', _95 => _95.throwIfAborted, 'call', _96 => _96()]);
    ...
    _optionalChain([abortSignal, 'optionalAccess', _97 => _97.throwIfAborted, 'call', _98 => _98()]);
...
async function* listDatasets(params) {
  const accessToken = params && checkCredentials(params);
  let totalToFetch = _nullishCoalesce(_optionalChain([params, 'optionalAccess', _99 => _99.limit]), () => ( Infinity));
  const search = new URLSearchParams([
    ...Object.entries({
      limit: String(Math.min(totalToFetch, 500)),
      ..._optionalChain([params, 'optionalAccess', _100 => _100.search, 'optionalAccess', _101 => _101.owner]) ? { author: params.search.owner } : void 0,
      ..._optionalChain([params, 'optionalAccess', _102 => _102.search, 'optionalAccess', _103 => _103.query]) ? { search: params.search.query } : void 0
    }),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _104 => _104.search, 'optionalAccess', _105 => _105.tags, 'optionalAccess', _106 => _106.map, 'call', _107 => _107((tag) => ["filter", tag])]), () => ( [])),
    ...DATASET_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _108 => _108.additionalFields, 'optionalAccess', _109 => _109.map, 'call', _110 => _110((val) => ["expand", val])]), () => ( []))
  ]).toString();
  let url = `${_optionalChain([params, 'optionalAccess', _111 => _111.hubUrl]) || HUB_URL}/api/datasets` + (search ? "?" + search : "");
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _112 => _112.fetch]), () => ( fetch)))(url, {
      headers: {
    ...
    for (const item of items) {
      yield {
        ..._optionalChain([params, 'optionalAccess', _113 => _113.additionalFields]) && pick(item, params.additionalFields),
        id: item._id,
        name: item.id,
...
async function datasetInfo(params) {
  ...
  const search = new URLSearchParams([
    ...DATASET_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _114 => _114.additionalFields, 'optionalAccess', _115 => _115.map, 'call', _116 => _116((val) => ["expand", val])]), () => ( []))
  ]).toString();
  const response = await (params.fetch || fetch)(
    `${_optionalChain([params, 'optionalAccess', _117 => _117.hubUrl]) || HUB_URL}/api/datasets/${params.name}/revision/${encodeURIComponent(
  ...
  return {
    ..._optionalChain([params, 'optionalAccess', _118 => _118.additionalFields]) && pick(data, params.additionalFields),
    id: data._id,
    name: data.id,
...
async function fileDownloadInfo(params) {
  ...
  let size;
  let xetInfo;
  if (_optionalChain([resp, 'access', _119 => _119.headers, 'access', _120 => _120.get, 'call', _121 => _121("Content-Type"), 'optionalAccess', _122 => _122.includes, 'call', _123 => _123("application/vnd.xet-fileinfo+json")])) {
  ...
  return {
    ...
    url: resp.url && (new URL(resp.url).origin === new URL(hubUrl).origin || _optionalChain([resp, 'access', _124 => _124.headers, 'access', _125 => _125.get, 'call', _126 => _126("X-Cache"), 'optionalAccess', _127 => _127.endsWith, 'call', _128 => _128(" cloudfront")])) ? resp.url : url
  };
}
...
async function* listModels(params) {
  const accessToken = params && checkCredentials(params);
  let totalToFetch = _nullishCoalesce(_optionalChain([params, 'optionalAccess', _129 => _129.limit]), () => ( Infinity));
  const search = new URLSearchParams([
    ...Object.entries({
      limit: String(Math.min(totalToFetch, 500)),
      ..._optionalChain([params, 'optionalAccess', _130 => _130.search, 'optionalAccess', _131 => _131.owner]) ? { author: params.search.owner } : void 0,
      ..._optionalChain([params, 'optionalAccess', _132 => _132.search, 'optionalAccess', _133 => _133.task]) ? { pipeline_tag: params.search.task } : void 0,
      ..._optionalChain([params, 'optionalAccess', _134 => _134.search, 'optionalAccess', _135 => _135.query]) ? { search: params.search.query } : void 0,
      ..._optionalChain([params, 'optionalAccess', _136 => _136.search, 'optionalAccess', _137 => _137.inferenceProviders]) ? { inference_provider: params.search.inferenceProviders.join(",") } : void 0
    }),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _138 => _138.search, 'optionalAccess', _139 => _139.tags, 'optionalAccess', _140 => _140.map, 'call', _141 => _141((tag) => ["filter", tag])]), () => ( [])),
    ...MODEL_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _142 => _142.additionalFields, 'optionalAccess', _143 => _143.map, 'call', _144 => _144((val) => ["expand", val])]), () => ( []))
  ]).toString();
  let url = `${_optionalChain([params, 'optionalAccess', _145 => _145.hubUrl]) || HUB_URL}/api/models?${search}`;
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _146 => _146.fetch]), () => ( fetch)))(url, {
      headers: {
        accept: "application/json",
        ...accessToken ? { Authorization: `Bearer ${accessToken}` } : void 0
      }
    ...
    for (const item of items) {
      ...
      if (_optionalChain([params, 'optionalAccess', _147 => _147.additionalFields, 'optionalAccess', _148 => _148.includes, 'call', _149 => _149("inferenceProviderMapping")]) && item.inferenceProviderMapping) {
      ...
      yield {
        ..._optionalChain([params, 'optionalAccess', _150 => _150.additionalFields]) && pick(normalizedItem, params.additionalFields),
        id: item._id,
        name: item.id,
        private: item.private,
...
async function* listSpaces(params) {
  ...
  const search = new URLSearchParams([
    ...Object.entries({
      limit: "500",
      ..._optionalChain([params, 'optionalAccess', _151 => _151.search, 'optionalAccess', _152 => _152.owner]) ? { author: params.search.owner } : void 0,
      ..._optionalChain([params, 'optionalAccess', _153 => _153.search, 'optionalAccess', _154 => _154.query]) ? { search: params.search.query } : void 0
    }),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _155 => _155.search, 'optionalAccess', _156 => _156.tags, 'optionalAccess', _157 => _157.map, 'call', _158 => _158((tag) => ["filter", tag])]), () => ( [])),
    ...[...SPACE_EXPAND_KEYS, ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _159 => _159.additionalFields]), () => ( []))].map(
      (val) => ["expand", val]
    )
  ]).toString();
  let url = `${_optionalChain([params, 'optionalAccess', _160 => _160.hubUrl]) || HUB_URL}/api/spaces?${search}`;
  while (url) {
    const res = await (_nullishCoalesce(_optionalChain([params, 'optionalAccess', _161 => _161.fetch]), () => ( fetch)))(url, {
      headers: {
        accept: "application/json",
        ...accessToken ? { Authorization: `Bearer ${accessToken}` } : void 0
      }
    ...
    for (const item of items) {
      yield {
        ..._optionalChain([params, 'optionalAccess', _162 => _162.additionalFields]) && pick(item, params.additionalFields),
        id: item._id,
        name: item.id,
        sdk: item.sdk,
        likes: item.likes,
...
async function* listCollections(params) {
  ...
  let totalToFetch = _nullishCoalesce(_optionalChain([params, 'optionalAccess', _163 => _163.limit]), () => ( Infinity));
  searchParams.append("limit", String(Math.min(totalToFetch, 100)));
  if (_optionalChain([params, 'optionalAccess', _164 => _164.sort])) {
    searchParams.append("sort", params.sort);
  }
  if (_optionalChain([params, 'optionalAccess', _165 => _165.search, 'optionalAccess', _166 => _166.owner])) {
    for (const owner of params.search.owner) {
      searchParams.append("owner", owner);
    }
  }
  if (_optionalChain([params, 'optionalAccess', _167 => _167.search, 'optionalAccess', _168 => _168.item])) {
    for (const item of params.search.item) {
      searchParams.append("item", item);
    }
  }
  if (_optionalChain([params, 'optionalAccess', _169 => _169.search, 'optionalAccess', _170 => _170.q])) {
    searchParams.append("q", params.search.q);
  }
  let url = `${_optionalChain([params, 'optionalAccess', _171 => _171.hubUrl]) || HUB_URL}/api/collections?${searchParams}`;
  while (url) {
...
async function modelInfo(params) {
  ...
  const search = new URLSearchParams([
    ...MODEL_EXPAND_KEYS.map((val) => ["expand", val]),
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _173 => _173.additionalFields, 'optionalAccess', _174 => _174.map, 'call', _175 => _175((val) => ["expand", val])]), () => ( []))
  ]).toString();
  const response = await (params.fetch || fetch)(
    `${_optionalChain([params, 'optionalAccess', _176 => _176.hubUrl]) || HUB_URL}/api/models/${params.name}/revision/${encodeURIComponent(
      _nullishCoalesce(params.revision, () => ( "HEAD"))
    )}?${search.toString()}`,
    {
      headers: {
  ...
  if (_optionalChain([params, 'optionalAccess', _177 => _177.additionalFields, 'optionalAccess', _178 => _178.includes, 'call', _179 => _179("inferenceProviderMapping")]) && data.inferenceProviderMapping) {
    normalizedData.inferenceProviderMapping = normalizeInferenceProviderMapping(data.id, data.inferenceProviderMapping);
  }
  return {
    ..._optionalChain([params, 'optionalAccess', _180 => _180.additionalFields]) && pick(normalizedData, params.additionalFields),
    id: data._id,
    name: data.id,
    private: data.private,
...
async function oauthHandleRedirect(opts) {
  if (typeof window === "undefined" && !_optionalChain([opts, 'optionalAccess', _181 => _181.redirectedUrl])) {
    throw new Error("oauthHandleRedirect is only available in the browser, unless you provide redirectedUrl");
  }
  if (typeof localStorage === "undefined" && (!_optionalChain([opts, 'optionalAccess', _182 => _182.nonce]) || !_optionalChain([opts, 'optionalAccess', _183 => _183.codeVerifier]))) {
    throw new Error(
      "oauthHandleRedirect requires localStorage to be available, unless you provide nonce and codeVerifier"
    );
  }
  const redirectedUrl = _nullishCoalesce(_optionalChain([opts, 'optionalAccess', _184 => _184.redirectedUrl]), () => ( window.location.href));
  const searchParams = (() => {
    try {
  ...
  const nonce = _nullishCoalesce(_optionalChain([opts, 'optionalAccess', _185 => _185.nonce]), () => ( localStorage.getItem("huggingface.co:oauth:nonce")));
  if (!code) {
  ...
  const codeVerifier = _nullishCoalesce(_optionalChain([opts, 'optionalAccess', _186 => _186.codeVerifier]), () => ( localStorage.getItem("huggingface.co:oauth:code_verifier")));
  if (!codeVerifier) {
  ...
  const hubUrl = _optionalChain([opts, 'optionalAccess', _187 => _187.hubUrl]) || HUB_URL;
  ...
  if (!_optionalChain([opts, 'optionalAccess', _188 => _188.codeVerifier])) {
    localStorage.removeItem("huggingface.co:oauth:code_verifier");
  }
  if (!_optionalChain([opts, 'optionalAccess', _189 => _189.nonce])) {
...
async function oauthHandleRedirectIfPresent(opts) {
  if (typeof window === "undefined" && !_optionalChain([opts, 'optionalAccess', _190 => _190.redirectedUrl])) {
    throw new Error("oauthHandleRedirect is only available in the browser, unless you provide redirectedUrl");
  }
  if (typeof localStorage === "undefined" && (!_optionalChain([opts, 'optionalAccess', _191 => _191.nonce]) || !_optionalChain([opts, 'optionalAccess', _192 => _192.codeVerifier]))) {
    throw new Error(
      "oauthHandleRedirect requires localStorage to be available, unless you provide nonce and codeVerifier"
    );
  }
  const searchParams = new URLSearchParams(_nullishCoalesce(_optionalChain([opts, 'optionalAccess', _193 => _193.redirectedUrl]), () => ( window.location.search)));
...
async function oauthLoginUrl(opts) {
  if (typeof window === "undefined" && (!_optionalChain([opts, 'optionalAccess', _194 => _194.redirectUrl]) || !_optionalChain([opts, 'optionalAccess', _195 => _195.clientId]))) {
    throw new Error("oauthLogin is only available in the browser, unless you provide clientId and redirectUrl");
  }
  if (typeof localStorage === "undefined" && !_optionalChain([opts, 'optionalAccess', _196 => _196.localStorage])) {
    throw new Error(
  ...
  const hubUrl = _optionalChain([opts, 'optionalAccess', _197 => _197.hubUrl]) || HUB_URL;
  const openidConfigUrl = `${new URL(hubUrl).origin}/.well-known/openid-configuration`;
  const openidConfigRes = await fetch(openidConfigUrl, {
    headers: {
      Accept: "application/json"
  ...
  if (_optionalChain([opts, 'optionalAccess', _198 => _198.localStorage])) {
  ...
  const redirectUri = _optionalChain([opts, 'optionalAccess', _199 => _199.redirectUrl]) || (typeof window !== "undefined" ? window.location.href : void 0);
  if (!redirectUri) {
  ...
  const state = JSON.stringify({
    nonce: newNonce,
    redirectUri,
    state: _optionalChain([opts, 'optionalAccess', _200 => _200.state])
  });
  const variables = (
    // @ts-expect-error window.huggingface is defined inside static Spaces.
    typeof window !== "undefined" ? _nullishCoalesce(_optionalChain([window, 'access', _201 => _201.huggingface, 'optionalAccess', _202 => _202.variables]), () => ( null)) : null
  );
  const clientId = _optionalChain([opts, 'optionalAccess', _203 => _203.clientId]) || _optionalChain([variables, 'optionalAccess', _204 => _204.OAUTH_CLIENT_ID]);
  if (!clientId) {
    if (variables) {
  ...
  return `${opendidConfig.authorization_endpoint}?${new URLSearchParams({
    client_id: clientId,
    scope: _optionalChain([opts, 'optionalAccess', _205 => _205.scopes]) || _optionalChain([variables, 'optionalAccess', _206 => _206.OAUTH_SCOPES]) || "openid profile",
    response_type: "code",
...
async function parseSafetensorsMetadata(params) {
  ...
  const quantConfig = _optionalChain([modelConfig, 'optionalAccess', _207 => _207.quantization_config]);
  if (params.path && RE_SAFETENSORS_FILE.test(params.path) || await fileExists({ ...params, path: SAFETENSORS_FILE })) {
    ...
    return {
      ...
      ...params.computeParametersCount ? {
        ...
        parameterTotal: (
          /// shortcut: get param count directly from metadata
          _optionalChain([index, 'access', _208 => _208.metadata, 'optionalAccess', _209 => _209.total_parameters]) ? typeof index.metadata.total_parameters === "number" ? index.metadata.total_parameters : typeof index.metadata.total_parameters === "string" ? parseInt(index.metadata.total_parameters) : void 0 : void 0
        )
      } : void 0
    };
  } else {
...
async function spaceInfo(params) {
  ...
  const search = new URLSearchParams([
    ...
    ..._nullishCoalesce(_optionalChain([params, 'optionalAccess', _210 => _210.additionalFields, 'optionalAccess', _211 => _211.map, 'call', _212 => _212((val) => ["expand", val])]), () => ( []))
  ]).toString();
  const response = await (params.fetch || fetch)(
    `${_optionalChain([params, 'optionalAccess', _213 => _213.hubUrl]) || HUB_URL}/api/spaces/${params.name}/revision/${encodeURIComponent(
      _nullishCoalesce(params.revision, () => ( "HEAD"))
    )}?${search.toString()}`,
    {
      headers: {
  ...
  const data = await response.json();
  return {
    ..._optionalChain([params, 'optionalAccess', _214 => _214.additionalFields]) && pick(data, params.additionalFields),
    id: data._id,
    name: data.id,
    sdk: data.sdk,
    likes: data.likes,
...
async function* uploadFilesWithProgress(params) {
  return yield* commitIter({
    ...
    fetch: params.useXet === true ? (
      ...
      _optionalChain([init, 'access', _215 => _215.signal, 'optionalAccess', _216 => _216.throwIfAborted, 'call', _217 => _217()]);
      xhr.send(init.body);
      return new Promise((resolve2, reject) => {
        ...
        if (init.signal) {
          init.signal.addEventListener("abort", () => {
            xhr.abort();
            try {
              _optionalChain([init, 'access', _218 => _218.signal, 'optionalAccess', _219 => _219.throwIfAborted, 'call', _220 => _220()]);
            } catch (err) {
              reject(err);
            }
          });
...
async function whoAmI(params) {
  ...
  if (typeof _optionalChain([response, 'access', _221 => _221.auth, 'access', _222 => _222.accessToken, 'optionalAccess', _223 => _223.createdAt]) === "string") {
