...
async function* sha256(buffer, opts) {
  ...
  if (!cryptoModule) {
    cryptoModule = await import("./sha256-node-HHCAVRWX.mjs");
  }
...
async function createBlobs(url, destPath, opts) {
  ...
  if (url.protocol === "file:") {
    const { FileBlob } = await import("./FileBlob-YDXJOHML.mjs");
    const { subPaths } = await import("./sub-paths-HAS7QVIV.mjs");
...
async function* createXorbs(fileSources, params) {
  const chunkModule = await import("./chunker_wasm-HQXI32MH.mjs");
  let xorbId = 0;
